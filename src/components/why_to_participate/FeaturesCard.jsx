    ...
    <>
     {
      state ? 
       (
      <div
      className={`-ml-6 grid gap-1 md:gap-[14px] m-0 md:m-4 py-7 md:w-[100%]
      ...
      transition-all`} 
    >
      <h3 className='text-md text-center sm:text-4xl md:text-2xl font-semibold px-3
       hover:text-purple pb-3'>{props.header}</h3>
      <p className='text-xs md:text-md xl:text-lg px-0 md:px-5'>{props.description}</p>
    </div>)
      :  
      (
        <div
        className={`grid gap-1 md:gap-[14px] m-0 md:m-4 py-7
        md:w-[100%] h-[200px] md:h-[250px] lg:h-[320px] w-fit px-6
        xl:h-[260px] sm:w-fit col-span-1 
